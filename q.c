...
@brief      This file contains code for sorting cash denominations, and there are three 
            functions used to verify, count and print: print_line(), coins(), main().
            The main() function will prompt the user for inputs and verifies it according 
            to certain conditions. It then calls the coins() function which counts the 
            number of denominations and calls the print_line() funtion. It then prints 
            out a table after aligning and adding sufficient padding.
...
/*!
@brief  main() function reads the inputs from the user and verifies it. It uses If Else 
        statements to check the inputs. A proper input would look like: 0.05 or 1.0 
        or 12.34 
        An inproper input would look like: x or " " or 1 or -5 or 5.2358
...
int main(void)
{
    ...
    else if (check != 2)   { //checks for invalid input formatting
